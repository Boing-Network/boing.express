...
- **Done:** **`QaPoolVote`** (`type: "qa_pool_vote"`) for observer `/qa` rewarded voting — `subject` (pending deploy tx hash) + `vote` allow|reject|abstain; suggested access list is sender read/write.
