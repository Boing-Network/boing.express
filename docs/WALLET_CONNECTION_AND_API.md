...
- **`type`:** `transfer` \| `bond` \| `unbond` \| `claim_unbond` \| `qa_pool_vote` \| `contract_call` \| `contract_deploy_purpose` \| `contract_deploy_meta` (not legacy `contract_deploy`).
- **`qa_pool_vote`:** `subject` (32-byte hex of the Unsure deploy tx id) and `vote` (`allow` \| `reject` \| `abstain`). Counted Allow/Reject votes are paid from the protocol treasury when the signed vote is included. The dApp should use **`boing_sendTransaction`** so the user signs and submits.
- **`access_list` / `accessList`:** optional. When omitted, the wallet fills the protocol’s suggested parallel access list for `transfer` / `bond` / `unbond` / `claim_unbond` / `qa_pool_vote` / `contract_call`. Deploy types still default to empty unless the dApp supplies one (simulation may return `suggested_access_list`).
