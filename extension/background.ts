...
async function signOrSendBoingTransaction(
  ...
  if (txReq == null || typeof txReq !== 'object') {
    throw providerError(
      PROVIDER_ERROR_CODES.INVALID_PARAMS,
      'BOING_INVALID_TX',
      'Pass one argument: a transaction object (type: transfer | bond | unbond | claim_unbond | qa_pool_vote | contract_call | contract_deploy_purpose | contract_deploy_meta | …). Deployments must declare a valid purpose_category (protocol QA). QA votes use type qa_pool_vote with subject (pending deploy tx hash) and vote allow|reject|abstain.'
    );
  }
